                ...
                >
                  <div className="flex justify-between items-center">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h4 className="font-bold text-heading text-lg">{app.firstName} {app.lastName}</h4>
            ...
            <AnimatePresence mode="wait">
              {selectedApp ? (
                <motion.div 
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  className="bg-white rounded-[2.5rem] p-8 shadow-2xl shadow-gray-200/50 sticky top-12 border border-gray-100 overflow-y-auto max-h-[calc(100vh-100px)] custom-scrollbar"
                >
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <p className="text-lg font-bold text-heading">{selectedApp.firstName} {selectedApp.lastName} ({selectedApp.age})</p>
                      <p className="text-sm font-medium text-body">{selectedApp.residence} • {selectedApp.gender}</p>
                    </div>

                    <div className="p-4 bg-gray-50 rounded-2xl">
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Social Handles</label>
                      <div className="flex flex-wrap gap-2">
                        {selectedApp.socialHandles && Object.entries(selectedApp.socialHandles).map(([platform, handle]: any) => (
                          <div key={platform} className="px-3 py-1 bg-white border border-gray-200 rounded-lg text-[10px] font-bold text-heading">
                            {platform}: {handle}
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 bg-gray-50 rounded-2xl">
                         <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Gifted Collabs</label>
                         <p className="text-sm font-bold text-primary">{selectedApp.acceptsGifted}</p>
                      </div>
                      <div className="p-4 bg-gray-50 rounded-2xl">
                         <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">TAT</label>
                         <p className="text-sm font-bold text-primary">{selectedApp.tat}</p>
                      </div>
                    </div>

                    <div>
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Interests</label>
                      <div className="flex flex-wrap gap-2">
                        {selectedApp.interestFields?.map((field: string) => (
                          <span key={field} className="px-3 py-1 bg-accent/20 text-primary rounded-full text-[10px] font-bold">
                            {field}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Portfolio</label>
                      <a href={selectedApp.portfolioLink} target="_blank" className="text-xs font-bold text-primary underline flex items-center">
                        View Portfolio <ExternalLink size={12} className="ml-1" />
                      </a>
                    </div>

                    <div>
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-3">License & Docs</label>
                      <div className="space-y-2">
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100">
                          <span className="text-[10px] font-bold text-heading">License ({selectedApp.hasLicense})</span>
                          {selectedApp.hasLicense === 'Yes' && <FileText size={14} className="text-primary" />}
                        </div>
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100">
                          <span className="text-[10px] font-bold text-heading">Emirates ID / Passport</span>
                          <Check size={14} className="text-emerald-500" />
                        </div>
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100">
                          <span className="text-[10px] font-bold text-heading">Visa Copy</span>
                          <Check size={14} className="text-emerald-500" />
                        </div>
                      </div>
                    </div>
